"""
基于动态时空特征的节点重要性度量——小波变换（Wavelet Transform）

@File    :   WaveletTransform.py
@Time    :   2024/12/17 14:06:24
...
def compute_node_influence_wavelet(traffic_data, wavelet='db4'):
    """
    ...
    for i in range(traffic_data.shape[1]):
        # 对每个节点的流量数据进行小波变换
        coeffs = pywt.wavedec(traffic_data[:, i], wavelet)
        # 计算小波系数的标准差，作为影响力的度量
        node_influence.append(np.std(coeffs[0]))  # 只使用低频系数
    return np.array(node_influence)
...
def train_and_evaluate_with_node_influence(X_train, y_train, X_test, y_test, adj_matrix, traffic_data, scaler, epochs,
                                           ...
                                           patience_reduce_lr):
    # 计算节点影响力
    node_influence = compute_node_influence_wavelet(adj_matrix)

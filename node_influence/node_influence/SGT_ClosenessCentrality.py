"""
谱图理论（Spectral Graph Theory）——传播中心性（Closeness Centrality）

@File    :   SGT_ClosenessCentrality.py
@Time    :   2024/12/17 14:04:45
...
def compute_node_influence_closeness(adj_matrix):
    """
    ...
    return np.array(list(closeness.values()))
...
def train_and_evaluate_with_node_influence(X_train, y_train, X_test, y_test, adj_matrix, traffic_data, scaler, epochs,
                                           ...
                                           patience_reduce_lr):
    # 计算节点影响力
    node_influence = compute_node_influence_closeness(adj_matrix)

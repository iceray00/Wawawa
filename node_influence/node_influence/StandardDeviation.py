"""
标准差——直接利用标准差作为计算节点影响力的依据

@File    :   StandardDeviation.py
@Time    :   2024/12/17 14:10:36
...
def compute_node_influence(traffic_data, window_size=12):
    """
    ...
    """
    node_influence = np.std(traffic_data, axis=0)  # 计算每个节点的标准差，作为影响力
...
def train_and_evaluate_with_node_influence(X_train, y_train, X_test, y_test, adj_matrix, traffic_data, scaler, epochs,
                                           ...
                                           patience_reduce_lr):
    # 计算节点影响力
    node_influence = compute_node_influence(traffic_data, window_size)

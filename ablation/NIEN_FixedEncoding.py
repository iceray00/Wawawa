...
class FixedPositionalEncoding(tf.keras.layers.Layer):  # 使用普通的固定位置编码
    def __init__(self, max_len, d_model):
    ...
    def call(self, x):
        seq_len = tf.shape(x)[1]
        return x + self.fixed_pos_encoding[:seq_len]  # 只使用正余弦位置编码
...
def build_transformer_model(input_shape, adj_matrix, d_model=64, num_heads=4, ff_dim=128, num_layers=3, max_len=12,
    ...
    # 固定的位置编码
    position_encoding = FixedPositionalEncoding(max_len, d_model)(x)

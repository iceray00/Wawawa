...
NIEN --- Node Influence Enhanced Network

谱图理论（Spectral Graph Theory）——拉普拉斯矩阵变换

@File    :   NIEN.py
@Time    :   2024/12/17 13:59:06
...
def compute_node_influence_with_spectral_graph(adj_matrix):
    """
    计算每个节点的影响力（基于流量的标准差，作为节点重要性的代理）。
    :param traffic_data: 交通流量数据，形状为 (时间步数, 节点数)
    :param window_size: 用于计算流量波动的窗口大小
    :return: 每个节点的影响力，形状为 (节点数,)
    """
    # 计算拉普拉斯矩阵的特征向量
    L = np.diag(np.sum(adj_matrix, axis=0)) - adj_matrix  # 计算拉普拉斯矩阵
    eigenvalues, eigenvectors = np.linalg.eigh(L)  # 求解特征值和特征向量
    # 选择第二小的特征值的特征向量
    second_smallest_eigenvector = eigenvectors[:, 1]
    return np.abs(second_smallest_eigenvector)  # 返回特征向量的绝对值作为影响力
...
def adjust_adjacency_matrix(adj_matrix, node_influence):
    ...
    :param node_influence: 节点影响力，形状为 (节点数,)
    :return: 加权后的邻接矩阵
    """
    # 对节点影响力进行归一化
    influence_norm = node_influence / np.max(node_influence)

    # 创建加权的邻接矩阵，基于节点影响力调整
    weighted_adj_matrix = adj_matrix * influence_norm
...
class StandardMultiHeadAttention(MultiHeadAttention):  # 普通的掩码
    def __init__(self, num_heads, key_dim, **kwargs):
        super(StandardMultiHeadAttention, self).__init__(num_heads=num_heads, key_dim=key_dim, **kwargs)
...
def transformer_block(inputs, d_model, num_heads, ff_dim, dropout_rate=0.1):
    ...
    # 标准掩码
    attn_layer = StandardMultiHeadAttention(num_heads=num_heads, key_dim=d_model)
...
def train_and_evaluate_with_node_influence(X_train, y_train, X_test, y_test, adj_matrix, traffic_data, scaler, epochs,
                                           ...
                                           patience_reduce_lr):
    # 计算节点影响力
    node_influence = compute_node_influence_with_spectral_graph(adj_matrix)
